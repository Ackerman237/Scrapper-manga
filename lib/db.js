 ...
 */
export function upsertPosition({ deviceId, mangaSlug, chapterId, page, chapterNum, mangaTitle, coverUrl }) {
  const stmt = db.prepare(`
    INSERT INTO reading_positions (device_id, manga_slug, chapter_id, page, chapter_num, manga_title, cover_url, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, unixepoch())
  ...
  `);
  stmt.run(deviceId, mangaSlug, chapterId, page || 1, chapterNum || null, mangaTitle || null, coverUrl || null);
}
...
export function getAllPositions({ deviceId }) {
  const stmt = db.prepare(`
    SELECT manga_slug, chapter_id, page, chapter_num, manga_title, cover_url, updated_at
    FROM reading_positions
    WHERE device_id = ?
